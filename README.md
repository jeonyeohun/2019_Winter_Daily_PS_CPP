...
* 반례를 생각해보자. 만약 방문객수가 층수로 딱 떨어진다면? 만약 7층짜리 호텔에 방문객이 14명이라고 치면 2로 나누어 떨어지는데 이때는 1을 더하지 않고 2로 호수를 정하고, 층수는 나머지가 0이 나올 때, 전체 층수로 바꿔주면 될 것 같은데..? 일단 코드 짜보고 반례나오면 더 
